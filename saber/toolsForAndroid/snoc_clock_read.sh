...
start_time=$(date +%s)
cur_time=$(date +%s)
file_name="snoc_freq_"$start_time.txt
...
display_freq() {
	if [ -f /sys/kernel/debug/clk/measure_only_snoc_clk/clk_measure ]; then
    		echo "$(date +%s): " $(< /sys/kernel/debug/clk/measure_only_snoc_clk/clk_measure) >> $log_file_path
  	else
